...
export default function SupervisorDetail() {
  ...
  const handleDelete = async () => {
    ...
    try {
      await api.delete(`/supervisors/${sid}/permanent`);
  ...
  if (loading) return (
    <OwnerLayout title="Supervisor Detail">
  ...
  if (!supervisor) return <OwnerLayout title="Supervisor Detail"><div className="p-8 text-center text-gray-400">Supervisor not found</div></OwnerLayout>;
                ...
                <table className="w-full text-sm min-w-[600px]">
                  ...
                  <tbody>
                    {paginatedEvents.map(e => (
                      <tr key={e.id} onClick={() => nav(`/provider/events/${e.id}`)} className="border-t border-gray-100 hover:bg-gray-50 cursor-pointer transition-colors group">
