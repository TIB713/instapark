...
export default function Cars() { 
  ...
  const fetchCars = async () => {
    setLoading(true);
    try {
      const { data } = await api.get("/superadmin/cars", {
  ...
  return ( 
    <SuperLayout title="Cars Registry"> 
      <div className="mb-6"> 
        <h1 className="font-heading text-2xl font-bold text-[#0F2044]">Cars Registry</h1> 
        <p className="text-gray-500 text-sm">Every vehicle that has ever used InstaPark valet — across all providers and events.</p> 
      </div> 
              ...
              <tbody> 
                {filtered.map(r => ( 
                  <tr key={r.car_id || r.plate} 
                    onClick={() => nav(r.has_plate_issue ? `/superadmin/cars/id/${r.car_id}` : `/superadmin/cars/${encodeURIComponent(r.plate)}`)} 
