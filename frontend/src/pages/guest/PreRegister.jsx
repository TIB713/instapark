...
import { Car, User, Phone, Calendar, MapPin, ChevronDown, Loader2, Info } from "lucide-react"; 
...
export default function PreRegister() { 
  ...
  useEffect(() => { 
    api.get(`/pre-register/${providerToken}`) 
      .then(r => { 
        setPageData(r.data); 
  ...
  const submit = async () => {
    ...
    try { 
      const expectedArrivalUTC = form.expected_arrival 
        ...
        : ""; 
      const result = await api.post(`/pre-register/${providerToken}`, { 
        ...form, 
  ...
  if (invalid) return ( 
    <div className="min-h-screen bg-gradient-to-br from-[#0F2044] to-[#1A3C6E] flex items-center justify-center p-4"> 
      <div className="bg-white rounded-3xl p-8 max-w-sm w-full text-center"> 
        ...
        <p className="text-gray-500 text-sm">This registration link is not valid. Please contact the event organizer.</p> 
      </div> 
    </div> 
  ); 
    ...
    <div className="min-h-screen bg-gradient-to-br from-[#0F2044] to-[#1A3C6E] flex items-center justify-center p-4"> 
      <div className="bg-white rounded-3xl p-8 max-w-sm w-full text-center"> 
        ...
        {passToken && ( 
          <a 
            href={`/pass/${passToken}`} 
            className="block w-full bg-[#1A3C6E] text-white rounded-2xl py-4 font-bold 
            text-sm tracking-wider text-center mt-4 hover:bg-[#0F2044] transition" 
          > 
            VIEW MY QR CODE → 
          </a> 
        )} 
  ...
  const inp = "w-full border border-gray-200 rounded-xl px-4 py-3 text-sm focus:outline-none focus:border-[#1A3C6E] bg-white"; 
  ...
  return ( 
    <div className="min-h-screen bg-gradient-to-br from-[#0F2044] to-[#1A3C6E] flex items-center justify-center p-4"> 
      <div className="bg-white rounded-3xl shadow-2xl w-full max-w-md overflow-hidden"> 
        {/* Header */} 
        <div className="bg-gradient-to-r from-[#0F2044] to-[#1A3C6E] p-6 text-white text-center"> 
          <div className="w-14 h-14 bg-white/20 rounded-2xl mx-auto flex items-center justify-center mb-3"> 
            <Car className="w-7 h-7 text-white" /> 
          </div> 
          <h1 className="font-bold text-xl">Pre-Register Your Vehicle</h1> 
          <p className="text-white/80 text-sm mt-1">{pageData.provider.name}</p> 
        </div> 
        ...
        {!pageData.events || pageData.events.length === 0 ? (
          <div className="p-8 text-center">
            ...
            <h2 className="font-bold text-xl text-gray-800 mb-2">No Events Available</h2>
            <p className="text-gray-500 text-sm">
              There are no upcoming events at this time. Please contact the organizer for more information.
            </p>
          </div>
        ) : (
          <div className="p-6 space-y-4"> 
            ...
            <div> 
              {pageData.events.length === 1 ? (
                <>
                  <label className={lbl}>Selected Event</label> 
                  <div className="w-full border border-gray-100 rounded-xl px-4 py-3 text-sm bg-gray-50 font-semibold text-[#0F2044]">
                    {pageData.events[0].name} — {pageData.events[0].date} · {pageData.events[0].venue}
                  </div>
                </>
              ) : (
                <>
...
{ errors.expected_arrival && <p className="text-[11px] text-red-500 mt-1 font-medium">* {errors.expected_arrival}</p> } 
          ...
          <button onClick={submit} disabled={submitting} 
            className="w-full bg-[#1A3C6E] text-white rounded-2xl py-4 font-bold text-sm tracking-wider hover:bg-[#0F2044] transition disabled:opacity-60 mt-2"> 
            {submitting ? <Loader2 className="w-5 h-5 animate-spin mx-auto" /> : "REGISTER MY VEHICLE →"} 
          </button> 
